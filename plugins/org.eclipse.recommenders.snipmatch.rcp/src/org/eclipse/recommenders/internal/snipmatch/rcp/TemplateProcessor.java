...
public class TemplateProcessor {
    ...
    public void insertTemplate(Snippet snippet) {
        AbstractTextEditor activeEditor = (AbstractTextEditor) PlatformUI
                .getWorkbench().getActiveWorkbenchWindow().getActivePage()
                .getActiveEditor();
        activeEditor.setFocus();
        ISourceViewer sourceViewer = (ISourceViewer) activeEditor
                .getAdapter(ITextOperationTarget.class);
        Point range = sourceViewer.getSelectedRange();
        Template template = new Template("", "", contexId, snippet.getCode(),
                true);
        IRegion region = new Region(range.x, range.y);
        ICompilationUnit cu = (ICompilationUnit) EditorUtility
                .getEditorInputJavaElement(activeEditor, false);
        Position p = new Position(range.x, range.y);
        TemplateContext ctx = new JavaContext(javaContextType,
                sourceViewer.getDocument(), p, cu);
        TemplateProposal proposal = new TemplateProposal(template, ctx, region,
                null);
    ...
    private TemplateContextType createContextType() {
        ...
        addGlobalTemplateVariableResolvers(contextType);
