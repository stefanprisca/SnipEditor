...
package org.eclipse.recommenders.snipeditor.ui.core;

import java.util.HashMap;

/**
 * This class represents a match "environment", which is responsible for getting
 * code context information and integrating the returned matches. User should
 * extend this class.
 */
public abstract class MatchEnvironment {
    ...
    /**
     * Returns n array of the friendly names of the major types handled by this
     * environment.
     * 
     * @return An array of the friendly names of the major types handled by this
     *         environment.
     */
     ...
     * @param query
     *            The search query.
     * @return A hash map containing the interpretations for each token in the
     *         query.
     */
    ...
    /**
     * Tests a match returned by the search to see if it is valid within the
     * current environment.
     * 
     * @param match
    ...
    /**
     * Reset the match environment to its default state. For example, all code
    ...
    /**
     * Returns an array of completions for one argument. Must be implemented by
     * derived classes.
     * 
     * @param argNode
    ...
    /**
     * Returns the best possible interpretation of the given token's type, in
     * the current context.
     * 
     * @param token
    ...
    /**
     * Evaluate one match. The evaluated result of the root match node is
     * applied to the code environment.
     * 
     * @param node
    ...
    protected Object evaluateMatchNode(MatchNode node, boolean overview) {
        if (node instanceof EffectMatchNode) {
            ...
            if (arg.getParameter().getValue() == null
                    || arg.getParameter().getValue().isEmpty())
                if (overview)
    ...
    /**
     * Evaluates one effect with the given arguments. Must be implemented by
     * derived classes.
     * 
     * @param effectNode
     ...
     * @return The result of the evaluation.
     */
    protected abstract Object evaluateEffect(EffectMatchNode effectNode,
            Object [] args);
     ...
     * @throws Exception
     */
    protected abstract void applyResult(Object result, Effect effect)
            throws Exception;
